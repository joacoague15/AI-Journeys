...
const CharacterStatus = ({ characterStatuses }) => {
    const { health, maxHealth, mana, maxMana, attack } = characterStatuses;
    const [statusCharacter, setStatusCharacter] = useState()
    const [percentageHealth, setPercentageHealth] = useState()

    useEffect(() => {
        setStatusCharacter(IDBTransactionGetAll('attributes'))
        setPercentageHealth((statusCharacter?.constitution / statusCharacter?.maxConstitution) * 100);
    }, [])
        ...
        return (
            <div className="animate__animated animate__flipInX" style={{ position: "absolute", display: "flex", top: 10, left: 10 }}>
                <div style={{ margin: 15 }}>
                    ...
                    <p style={{ color: "white", fontSize: 28 }}>{statusCharacter?.constitution} / {statusCharacter?.maxConstitution}</p>
                </div>
                <div style={{ margin: 15 }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="white"
                    ...
                    <p style={{ color: "white", fontSize: 28 }}>{statusCharacter?.intelligence} / {statusCharacter?.maxIntelligence}</p>
                </div>
                <div style={{ margin: 15 }}>
                    <i style={{ color: "white", fontSize: 50 }} className="fa-solid fa-khanda"></i>
                    <p style={{ color: "white", fontSize: 28 }}>{statusCharacter?.strength}</p>
                </div>
            </div>
        )
    }

    if (percentageHealth >= 60) {
        return (
