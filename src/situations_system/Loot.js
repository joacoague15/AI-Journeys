...
import LootChest from "../images/chest.jpg";

const Loot = ({ setSituation }) => {
    const [loot, setLoot] = useState()
    const [modal, setModal] = useState(false)
    useEffect(() => {
        axios.get(`inventory_system/Inventory.JSON`, {
            headers: {
                'Content-Type': 'application/json'
            }
        }).then((response) => {
            return setLoot(response.data)
        })
    }, [])
    ...
    const getItem = () => {
        ...
        if (!modal) {
            setModal(true)
            if (wichLoot === 1) return setLoot(loot[0].weapons[Math.floor(Math.random() * (13 - min) + min)]);
            return setLoot(loot[1].armor[Math.floor(Math.random() * (8 - min) + min)])
        }
    }

    return (
        <><div style={{ width: '35%', position: 'absolute', left: '50%', bottom: 10, transform: 'translate(-50%)', height: '80%' }}>
            <img src={LootChest} alt="lootchest" style={{ maxWidth: '75%', left: '50%', position: 'absolute', transform: 'translate(-50%)', zIndex: 1, borderRadius: '12px' }} onClick={() => getItem()} />
